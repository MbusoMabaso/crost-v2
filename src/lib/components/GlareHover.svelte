...
>
	<div
		bind:this={overlay}
		style="position:absolute;inset:0;border-radius:{borderRadius};background:linear-gradient({glareAngle}deg, hsla(0,0%,0%,0) 60%, {rgba} 70%, hsla(0,0%,0%,0) 100%);background-size:{glareSize}% {glareSize}%, 100% 100%;background-repeat:no-repeat;background-position:-100% -100%, 0 0;pointer-events:none;z-index:1;"
	></div>
